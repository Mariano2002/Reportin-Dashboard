...
from Styling import table_layout, template1, template2, template3, template4, template5, template6, template7,\
    template8,bring_to_front, mi_script, table_style, css3
...
def table1():
    ...
    # Fixme: include billable bips in table
...
def table2():
    ...
    top10institutions = top10institutions.style.applymap(color_negative_red, subset=['Balance Change (%)','Balance Change ($)']).format({'Balance Change ($)':'${0:,.2f} MM', 'Balance Change (%)':'{0:.2f}%', 'Balance USDE':'${0:,.2f} MM'}).set_table_styles(table_layout)
...
def table3():
    top10providers = pd.pivot_table(df_master_for_panel_current_dt, values=['BALANCE_USDE'], index= ['BALANCE_DT','PARENT_FUND_PROVIDER', 'RATE_TYPE'], aggfunc=np.sum)
    top10providers.groupby(level=['PARENT_FUND_PROVIDER', 'RATE_TYPE'])

    hist_top10providers = pd.pivot_table(df_master_for_panel, values=['BALANCE_USDE'], index= ['BALANCE_DT','PARENT_FUND_PROVIDER', 'RATE_TYPE'], aggfunc=np.sum)
    top10providers.groupby(level=['PARENT_FUND_PROVIDER', 'RATE_TYPE'])

    top10providers['BALANCE_DAY_CHANGE_PERCENT'] = hist_top10providers.groupby(level=['PARENT_FUND_PROVIDER', 'RATE_TYPE'])['BALANCE_USDE'].pct_change(periods=1) *100
    top10providers['BALANCE_DAY_CHANGE'] = (hist_top10providers.groupby(level=['PARENT_FUND_PROVIDER', 'RATE_TYPE'])['BALANCE_USDE'].shift(periods=0) - hist_top10providers.groupby(level=['PARENT_FUND_PROVIDER', 'RATE_TYPE'])['BALANCE_USDE'].shift(periods=1))/1000000
    ...
    top10providers = top10providers.style.applymap(color_negative_red, subset=['Balance Change (%)','Balance Change ($)']).format({'Balance Change ($)':'${0:,.2f} MM', 'Balance Change (%)':'{0:.2f}%', 'Balance USDE':'${0:,.2f} MM'}).set_table_styles(table_layout)
...
provider_netActivity_pvt_head = provider_netActivity_pvt_head.style.applymap(color_negative_red, subset=['Net Activity']).format({'Net Activity':'${0:,.2f} K'}).set_table_styles(table_layout)

provider_netActivity_pvt_tail = provider_netActivity_pvt_tail.style.applymap(color_negative_red, subset=['Net Activity']).format({'Net Activity':'${0:,.2f} K'}).set_table_styles(table_layout)
...
top5providers = top5providers.style.applymap(color_negative_red, subset=['Balance Change (%)','Balance Change ($)']).format({'Balance Change ($)':'${0:,.2f} MM', 'Balance Change (%)':'{0:.2f}%', 'Balance USDE':'${0:,.2f} MM'}).set_table_styles(table_layout)


bot5providers = bot5providers.style.applymap(color_negative_red, subset=['Balance Change (%)','Balance Change ($)']).format({'Balance Change ($)':'${0:,.2f} MM', 'Balance Change (%)':'{0:.2f}%', 'Balance USDE':'${0:,.2f} MM'}).set_table_styles(table_layout)
...
# fixme: look inot pdf converter
subprocess.call([r"python", "converter.py", 'rev_dashboard_pdf.html', 'rev_dashboard.pdf'], shell=False)

#-------sending report via email---------

fromaddr = "fundconnect@globallink.com"
#final_recipients = "anussbaum@statestreet.com,btreves@globallink.com,eric_wolf@statestreet.com,KCConn@StateStreet.com,gpfortuna@statestreet.com,SDawson1@StateStreet.com,mjshapcott@statestreet.com,GPennycooke@StateStreet.com"
test_recipients = "DRiegert@Statestreet.com"

# variable to control recipients and whether email will be sent or not
# "test" will send email to test_recipients, "final" to final_recipients, "not" will send no email
status = "test"

def send_email(toaddr, fromaddr):
    msg = MIMEMultipart()
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Fund Connect Volume Revenue Dashboard "+latest_dt.strftime("%m-%d-%y")

    body = "Attached is the Fund Connect Performance Revenue Dashboard as of " + latest_dt.strftime("%m-%d-%y") + ":" + "\n$" + filtered_total_bal_usde_str + "B is the current balance and this is a change of " + filtered_total_bal_chg_str + "%" + " and $" + filtered_total_bal_chg_usd_str + "B." + "\n" + "\nAll values are expressed in USD. (Please open with Chrome for optimal performance.) \nWeb: http://btreves01-vdi.pc.ny2.eexchange.com:5006/panel_process"
...
# send email based on settings to test group, not at all or to final group
if status == "test":
    send_email(test_recipients, fromaddr)
elif status == "final":
    send_email(final_recipients, fromaddr)
elif status == "not":
    print("no email sent")
